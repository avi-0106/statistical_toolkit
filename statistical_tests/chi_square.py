...
class ChiSquareTest:
    ...
    def __init__(
        self,
        observed: Union[np.ndarray, pd.DataFrame, list[list]],
    ...
    def _validate_table(self):
        ...
        assert (
            self.observed.size > 0
        ), "Contingency table is empty after missing value handling"
    ...
    def plot_test(self):
        ...
        sns.heatmap(self.observed, annot=True, fmt="d", cmap="Blues", ax=ax[0])
        ax[0].set_title("Observed Frequencies")

        # Expected frequencies
        sns.heatmap(
            results["expected"], annot=True, fmt=".1f", cmap="Oranges", ax=ax[1]
        )
        ax[1].set_title("Expected Frequencies")

        plt.suptitle(
            f"Chi-square Test Results (φ={results['phi']:.2f}, V={results['cramers_v']:.2f})"
        )
        plt.tight_layout()
        plt.show()
